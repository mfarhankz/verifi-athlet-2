...
export default function Step1({ athlete, surveyData, onComplete }: Step1Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (athlete.sport_id) {
          try {
            const positionData = await fetchPositionsBySportId(athlete.sport_id.toString());
  ...
  const handleChange = (field: string, value: string | string[]) => {
    ...
    if (field === '26' || field === '29') { // Preferred contact method and Who will be helping
        ...
        <Flex vertical justify="center" align="center" className="text-center">
          ...
          <Typography.Text>
            Give us some basic information about yourself
               ...
               {/* Consent Checkbox */}
        <Flex vertical className="survey-textarea" style={{ marginBottom: '10px' }}>
         <Checkbox
           checked={formData['70'] === 'TRUE'}
           onChange={(e) => handleConsentChange(e.target.checked)}
         >
           By filling out this survey you consent to share your profile with all universities and colleges
         </Checkbox>
       </Flex>
         ...
         {loading ? (
           ...
           <>
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>What year in school are you?</Typography.Title>
               <Select
                 className="w-full"
                 value={formData[1]}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>Eligibility remaining (in years)?</Typography.Title>
               <Select
                 className="w-full"
                 value={formData[25]}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>Are you on scholarship?</Typography.Title>
               <Select
                 className="w-full"
                 value={formData[251]}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               ...
               <Select
                 ...
                 placeholder={positions.length === 0 ? "Loading positions..." : "Select position"}
               />
               {positions.length === 0 && (
             ...
             {/* Secondary Position */}
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>Secondary Position (if applicable)</Typography.Title>
               <Select
                 className="w-full"
                 value={formData[3]}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>Are you a grad transfer?</Typography.Title>
               <Select
                 className="w-full"
                 value={formData[570]}
            ...
            <Flex vertical className="mb-5 survey-textarea">
              <Typography.Title level={4}>Preferred contact method?</Typography.Title>
              <Select
                className="w-full"
                mode="multiple"
                value={formData[26] ? formData[26].split(', ').filter((v: string) => v.trim() !== '') : []}
             ...
             {(athlete.sport_id === 3 || athlete.sport_id === 4) && (
               <Flex vertical className="mb-5 survey-textarea">
                 <Typography.Title level={4}>International Phone Number</Typography.Title>
                 <Input 
                   value={formData[364]}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>When are you looking to transfer?</Typography.Title>
               <Select
                 className="w-full"
                 value={formData[28]}
                 onChange={(value) => handleChange('28', value)}
                 options={[
                   { value: "I want to transfer and play immediately", label: "I want to transfer and play immediately" },
                   { value: "I want to transfer asap but for the right situation I would wait to play", label: "I want to transfer asap but for the right situation I would wait to play" },
                   { value: "I'm waiting until the end of the school year and then will want to transfer", label: "I'm waiting until the end of the school year and then will want to transfer" },
                   { value: "I'm waiting until I graduate and then will want to transfer", label: "I'm waiting until I graduate and then will want to transfer" },
                 ]}
               />
             </Flex>
            ...
            <Flex vertical className="mb-5 survey-textarea">
              <Typography.Title level={4}>Who will be helping you with your decision?</Typography.Title>
              <Select
                className="w-full"
                mode="multiple"
                value={formData[29] ? formData[29].split(', ').filter((v: string) => v.trim() !== '') : []}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>Contact Info for anyone helping with your decision</Typography.Title>
               <Input 
                 value={formData[30]}
             ...
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>List any colleges you have attended prior to your current college</Typography.Title>
               <TextArea 
                 rows={4} 
                 value={formData[8]}
             ...
             {/* Home address */}
             <Flex vertical className="mb-5 survey-textarea">
               <Typography.Title level={4}>Home address (parents)</Typography.Title>
               <Input 
                 placeholder="Street"
                 value={formData[246]}
               ...
               <Flex vertical className="mb-5 survey-textarea">
                 <Typography.Title level={4}>Committed to Transfer to ... (If you have committed to transfer already, enter the school here.)</Typography.Title>
                 {loading ? (
                   ...
                   <Select
                     ...
                     onChange={(value) => handleChange('committed_school', value)}
                     showSearch
